...
\begin{figure}
  ...
  \caption{Combined Dice values for all labels of the evaluation between the two ANTs-based registration programs including affine-only transformations. \textcolor{red}{Perhaps we should also plot Dice but with the white matter and cerebellum labels removed since they're so large (could they be masking more noticeable differences?).}
  }
  \label{fig:results}
\end{figure}
\end{center}
