...
The neuroscience community has significantly benefited from the proliferation
...
Toolkit and our own Advanced Normalization Tools (ANTs),%
\footnote{
...
One of the stated reasons for using ANIMAL was ``because of our
...
Dovetailing with the previous item, developers should also facilitate
...
\subsection{Do not code up your own version of the algorithm particularly 
...
As a result, comparative evaluations are often carried out using 
...
the original authors) although not made public while the third, 
...
\subsection{Provide details as to the source of the algorithm}
...
into the Insight Toolkit for public dissemination.  However, since
...
code, there are subtle variations which can affect performance.
...
\subsection{Provide computational platform details of the evaluation}
An recent article \cite{gronenschild2012} pointed out significant
...
expected given upgrades and big fixes which occur between releases,
...
\subsection{Pre- and post-processing steps need to be supplied.}
...
\subsection{Resulting images should be publicly posted}
...
\subsection{Properly contextualize comparisons and motivate performance differences.}
...
assessment metrics.  In \cite{rohlfing2012}, Rohlfing pointed out 
...
taken by journal editorial boards are being used to reduce undeserving
...
However, as part of the recent ITKv4 effort, an improved
version for image registration, known as \verb#antsRegistration#
had been created and also released within ANTs \cite{avants2012}.
...
\lstset{frame = tb,
        ...
        language=,
        floatplacement=!h,
        caption={\small \baselineskip 12pt ANTS command line call used for the evaluation}.,
        captionpos=b,
        label=listing:ANTS
        }
\begin{lstlisting}
...
\lstset{frame = tb,
        ...
        language=,
        floatplacement=!h,
        caption={\small \baselineskip 12pt antsRegistration command line call used for the evaluation}.,
        captionpos=b,
...
\section{Conclusion}
\epigraph{By honest I don't mean that you only tell what's true. But you make clear the entire situation. You make clear all the information that is required for somebody else who is intelligent to make up their mind.}{Professor Richard Feynman}
