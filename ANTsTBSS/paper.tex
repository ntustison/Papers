...
Now, suppose the SSD metric is optimized to find a set of $M$ transformations that map a population of $M$ FA images, $\{I_1, I_2, \ldots, I_M\}$, to a representative FA template, $J$. Then, the algorithm will compute transformations
...
Eqn. \ref{eq:variance}, direct FA-to-FA registration will find transformations that align (or select) voxels such that the statistical testing result (or the effect of interest) is increased (assuming one is using Student's test).  While the SSD metric is able to provide visually attractive alignments across subjects, the voxel-level details (that is, which voxels are matched where) are, as we show in the following simulation study, strikingly important in terms of the effect on detection power.  
...
rank-sum testing (Fig. \ref{fig:simulated_testing}(b)).  The rank test does not explicitly use variance in its assessment and thus exhibits far less statistical exaggeration.  
