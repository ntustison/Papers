...
%% Text of abstract
Critical to many studies investigating white matter differences across populations 
is normalization of fractional anisotropy (FA) images to a common coordinate system
for statistical analysis.  Although variations exist, common to several current 
approaches is direct FA-to-FA registration whether it be to a chosen individual or
a generated template.  This type of circular analysis 
is susceptible to the general confounding 
effect of minimizing data differences with nonrigid registration which are potentially 
the precise differences to be statistically quantified.  We demonstrate that
population normalization to a template explicitly optimizes certain voxelwise statistical 
testing measures as opposed to maximizing anatomical alignment.  
As a corrective, we advocate the use of the Symmetric Group Normalization strategy for 
unbiased template construction from anatomical (e.g. T1-weighted) images which provides 
the coordinate system for statistical inference of FA differences.
Following template construction, individual FA images are warped to the template via 
the corresponding anatomical image.  Performance is evaluated using open data from the International 
Neuroimaging Data-sharing Initiative and a cohort of 16 traumatic brain injury survivors compared with 
17 control subjects.  Alignment comparisons with the normalization component of the 
popular tract-based spatial statistics (TBSS)
show that our anatomically-based template approach produces good alignments despite
the absence of any direct FA-to-FA registration.  Of practical consideration we note that the 
software tools discussed in this work are provided to the public as part of the open source 
...
The seminal work of Basser et al. \cite{Basser1994a,Basser1994} established diffusion tensor imaging (DTI) as a viable investigatory MRI technique.  Continued quantitative development has furthered its applicability to probing clinical questions of interest particularly pertaining to the microstructure of white matter \cite{Basser1996,Assaf2008}.  Given this sensitivity to brain architecture, DTI has found multiple uses in assessing neuro-structural differences in cross-population studies \cite{Kubicki2005,Arnone2008,Kantarci2010,Rametti2010}
including subjects with traumatic brain injury (TBI) \cite{Caeyenberghs2010,Jiang2010,Warner2010}.  

Two popular comprehensive software packages for assessing population FA differences include 
the SPM Matlab-based toolkit%
\footnote{
...
Although concerns regarding the general 
application of VBM \cite{Bookstein2001,Ashburner2001,Davatzikos2004}
and its application to FA data \cite{Jones2005,Chung2008} has been discussed 
in the literature, it continues to find application in FA population 
studies \cite{Kakeda2010,Takao2010}.
The TBSS framework was partially developed in response to these concerns
in which voxel values are projected onto a template white matter skeleton for 
...
commonality includes direct FA-to-FA registration.
Potential difficulties associated with direct FA-to-FA normalization is exemplified by 
a standard population study in which all subject FA images are mapped to a representative 
template using the popular sum of squared differences (SSD) image similarity metric.%
\footnote{
In addition to TBSS and SPM, other popular packages in the neuroimaging community 
which use the SSD metric includes AIR \cite{Woods1998} and LDDMM \cite{Beg2005}.
}  
Given a set of $M$ subject images, $\{I_1, I_2, \ldots, I_M\}$, and
a template image, $J$, population normalization concerns finding the set of $M$ transformations, 
...
\end{align}
Switching the order of the summations (since the transformations are mutually independent) and 
recognizing that a principal criterion for the selection of $J$ is such that it be a good approximation of the 
mean of the aligned images, it becomes apparent that the transformation solution
...
\end{align}
is that which minimizes the voxelwise variance or, equivalently,
maximizes various voxelwise statistical quantities including the common Student's $t$-test.%
Note that many of the standard similarity metrics are also going to exhibit this
effect to some degree as they minimize some local measure of intensity-based distance 
between the subject and the template.  

Thus, instead of normalization based on anatomical alignment followed by
statistical testing, direct FA normalization 
explicitly conflates anatomical alignment with optimizing certain statistical testing results
evoking a sense of circularity in the analysis \cite{Kriegeskorte2010}.
According to Kriegeskrote et al., ``An analysis is circular (or nonindependent) if it is based on
data that were selected for showing the effect of interest or a related effect.''  As illustrated with
Eqn. \ref{eq:variance}, direct FA-to-FA registration causes voxels to be matched (or selected) according to the
the statistical testing result (or the effect of interest) instead of anatomical alignment.
...
Gaussian smoothing (standard deviation = 1.5 mm).  A simulated patient cohort of 20 subjects
...
rate correction within the white matter to get ``true'' statistical significance 
curves.  These curves are plotted in black in Fig. \ref{fig:simulated_testing}.  To illustrate the 
effect of similarity metric on statistical significance, we performed ANTs registration
\cite{Avants2011} of each subject to the template
...
This example demonstrates the undesirable phenomenon previously described.  Even though these
images were anatomically aligned prior to registration, the resulting deformations and 
increased false statistical power can be explained in the context of Eqn. (\ref{eq:variance}).  
Common similarity metric functions can induce false positives by minimizing local intensity 
differences between the subjects and template which explicitly maximizes statistical
...
rank-sum testing (Fig. \ref{fig:simulated_testing}(b)) where as large of an effect for the latter would not be expected although an effect is
still apparent.
...
Although various processing choices mitigate these effects, such as Gaussian smoothing, nonparametric
testing, and
the use of more robust similarity metrics, such choices are often made on an ad hoc basis.  
...
of a high performance \cite{Klein2009}, nonrigid registration algorithm \cite{Avants2011}, which forms the underlying machinery for an optimal template construction algorithm \cite{Avants2010}, in facilitating alignment between DTI-derived scalar images for cross-population studies.  
...
An important step in reducing intersubject variability is
normalization to a standardized coordinate system for
cross-population comparison.  Various approaches
exist for determination of the normalized space such as selection of a
pre-existing template based on a single subject, e.g. the Talairach atlas \cite{Talairach1988}), or 
a publicly available averaged group of subjects, e.g. the MNI \cite{Collins1994} or 
ICBM \cite{Mazziotta1995} templates.

Recent trends, however, indicate a growing popularity of generating population-specific 
templates for minimizing potential confounds.  This includes the
diffeomorphic approach of Joshi et al. who employ the SSD metric and a shape 
distance to bring the subject group of images into alignment \cite{Joshi2004}.  Variants
...
templates \cite{Avants2010}. Given a set of representative images, 
...
updating of $\psi(\mathbf{x})$ by averaging the current estimate of $\left\{\phi_1^m\right\}$.  Additional details are given in \cite{Avants2010}.  
...
An example of our proposed workflow applied to anatomically-based population normalization prior to TBSS analysis  is illustrated in Fig. \ref{fig:workflow}.  Instead of direct FA-to-FA registration, our ANTs-based modification can be described as follows:
...
\begin{enumerate}
  ...
  \item Warp each subject's FA or MD map to the template using $\mathcal{T}^i_{total}$.
  \end{enumerate}      
...
Tensorial data associated with each of the following data sets were reconstructed from the diffusion weighted sequences using Camino%
\footnote{
http://camino.org.uk/
...
from the T1-weighted images of the two groups.  These two 
...
Statistical power curves (one-tailed, $\alpha = 0.95$)
...
It should be noted that although TBSS uses
the SSD metric, the fixed and moving images are smoothed and subsampled
even at the highest resolution level which substantially decreases the local effect
of the metric.
...
images is shown for each case in Figure \ref{fig:tbi_cov}.  This is
very much related to earlier findings in that strategies evincing
...
CDF.  Although direct FA-to-FA registration using ANTs with common
...
The mean FA image derived from the standard TBSS approach was thresholded at FA $\geq 0.2$. 
 ...
 producing a lower CoV for a given mean value.  By calculating the cumulative distribution 
 function (CDF) of the set of CoV values within the mask, quantitative assessment of alignment performance can be visualized.  
