...
priorWeights <- c( "0.0", "0.1", "0.25", "0.5" )


for( n in 1:length( priorWeights ) )
  {
  results <- read.csv( file = paste( "labelresults", priorWeights[n], ".csv", sep = '' ), header = TRUE );
  ...
  #                geom_smooth( aes( group = Gender, colour = Gender ), formula = y ~ 1 + x + I(x^2), method = "lm", size = 1, n = 1000, level = 0.95, se = TRUE, fill = 'black', alpha = 0.5 ) +
                 ...
                 scale_x_continuous( "Age (years)", breaks = seq( 20, 65, by = 10 ), labels = seq( 20, 65, by = 10 ), limits = c( 20, 65 ) ) +
                 scale_y_continuous( "Thickness (mm)", breaks = seq( 0, 10, by = 1 ), labels = seq( 0, 10, by = 1 ), limits = c( 0, 10 ) ) +
  ...
  #                scale_shape_discrete( breaks = c( 1, 2 ), labels = c( "Male", "Female" ) ) +
                 ...
                 ggtitle( paste( "Cortical thickness (", cortical_labels[i], ", priorWeight = ", priorWeights[n], ")", sep = "" ) )
    ggsave( filename = paste( "label", i, "_results", priorWeights[n], ".pdf", sep = "" ), plot = thickPlot, width = 8, height = 6, units = 'in' )
    }
  }
